...
def main():
    ...
    st.set_page_config(
        page_title="UATP Capsule Visualizer (Database)",
        page_icon="🔗",
        layout="wide"
    )
    
    st.title("🔗 UATP Capsule Visualizer (Database Edition)")
    st.markdown("Real-time visualization of Universal Attribution and Transparency Protocol capsules")
    ...
    use_database = st.sidebar.checkbox("Use Database", value=True, 
                                     help="Use SQLite database instead of JSONL fallback")
    ...
    with col4:
        if capsules:
            avg_significance = sum(c.get('significance_score', 0) for c in capsules) / len(capsules)
    ...
    # Significance filter
    min_significance = st.sidebar.slider(
        "Min Significance", 
        0.0, 10.0, 0.0, 0.1
    )
    ...
    if selected_platform != "All":
        filtered_capsules = [c for c in filtered_capsules if c.get('platform') == selected_platform]
    
    if min_significance > 0:
        filtered_capsules = [c for c in filtered_capsules if c.get('significance_score', 0) >= min_significance]
    ...
    # Main content tabs
    tab1, tab2, tab3, tab4 = st.tabs(["📊 Overview", "🔍 Capsule Explorer", "📈 Analytics", "⚙️ Database Info"])
...
def render_overview(capsules: List[Dict[str, Any]], stats: Dict[str, Any]):
    ...
    with col2:
        ...
        if significance_scores:
            ...
            fig.update_layout(
                ...
                yaxis_title="Number of Capsules"
            )
            st.plotly_chart(fig, use_container_width=True)
        else:
    ...
    recent_capsules = sorted(capsules, key=lambda x: x.get('timestamp', ''), reverse=True)[:10]
    ...
    if recent_capsules:
        for capsule in recent_capsules:
            with st.expander(f"🔗 {capsule.get('capsule_id', 'Unknown ID')[:20]}..."):
                ...
                with col2:
                    st.write(f"**Significance:** {capsule.get('significance_score', 0):.2f}")
                    st.write(f"**Timestamp:** {capsule.get('timestamp', 'Unknown')}")
                
                with col3:
                    st.write(f"**Type:** {capsule.get('type', 'Unknown')}")
                    st.write(f"**Storage:** {capsule.get('storage_backend', 'Unknown')}")
...
def render_analytics(capsules: List[Dict[str, Any]], stats: Dict[str, Any]]):
    ...
    for capsule in capsules:
        ...
        platform_data[platform]['total_significance'] += capsule.get('significance_score', 0)
    
    # Calculate averages
    for platform in platform_data:
        platform_data[platform]['avg_significance'] = (
            platform_data[platform]['total_significance'] / platform_data[platform]['count']
        )
...
def render_database_info(stats: Dict[str, Any]):
    ...
    if backend == "sqlite":
        st.success("✅ SQLite database is active")
    elif backend == "jsonl_fallback":
        st.warning("⚠️ Using JSONL fallback (database unavailable)")
    else:
        st.error("❌ Unknown storage backend")
