...
class CapsuleEmbedder:
    ...
    def __init__(self, database_url: Optional[str] = None):
        ...
        self.database_url = database_url or os.getenv(
            "DATABASE_URL", "postgresql://uatp_user@localhost:5432/uatp_capsule_engine"
        )
    ...
    def build_idf_from_corpus(self, texts: List[str]) -> None:
        ...
        logger.info(
            f"Built IDF from {self._doc_count} documents, {len(self._idf_cache)} unique terms"
        )
        ...
        try:
            with conn.cursor() as cur:
                ...
                for i in range(0, total, batch_size):
                    ...
                    # Save to database
                    for capsule, embedding in zip(
                        batch_capsules, embeddings, strict=False
                    ):
                        try:
                            cur.execute(
                                """
                    ...
                    conn.commit()
                    logger.info(
                        f"Processed {min(i + batch_size, total)}/{total} capsules"
                    )
