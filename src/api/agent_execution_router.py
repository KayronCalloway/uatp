...
class ToolCallRequest(BaseModel):
    ...
    completed_at: Optional[datetime] = Field(None, description="When tool call completed")
    duration_ms: Optional[int] = Field(None, ge=0, description="Duration in milliseconds")
    status: str = Field(default="success", description="Status: pending, success, error, timeout")
...
async def create_agent_session(
    ...
    logger.info(f"Agent session {result['session_id']} created for {request.agent_type}")
    return result
