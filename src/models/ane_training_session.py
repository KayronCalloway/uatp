...
class HardwareProfileModel(db.Base):
    ...
    # Relationships
    ane_sessions = relationship(
        "ANETrainingSessionModel", back_populates="hardware_profile"
    )
...
class ANETrainingSessionModel(db.Base):
    ...
    hardware_profile = relationship(
        "HardwareProfileModel", back_populates="ane_sessions"
    )
...
class KernelExecutionModel(db.Base):
    ...
    session = relationship(
        "ANETrainingSessionModel", back_populates="kernel_executions"
    )
    ...
    kernel_type = Column(
        String(50), nullable=False, index=True
    )  # KernelType values or Metal/MLX kernels
...
class CompileArtifactModel(db.Base):
    ...
    session = relationship(
        "ANETrainingSessionModel", back_populates="compile_artifacts"
    )
