...
try:
    ...
    _CONTRADICTION_ENGINE_AVAILABLE = True
except ImportError:
    _CONTRADICTION_ENGINE_AVAILABLE = False
    ContradictionEngine = None

# Import proof levels for honest labeling
try:
    from src.core.provenance import ProofLevel
...
class RichCaptureEnhancer:
    ...
    def create_capsule_from_session_with_rich_metadata(
        ...
        # CONTRADICTION SELF-INSPECTION
        # A protocol that can accuse itself is closer to truth.
        # Run contradiction detection to identify inconsistencies in our own output.
        if _CONTRADICTION_ENGINE_AVAILABLE and ContradictionEngine:
            try:
                ...
                if contradictions:
                    ...
                    if critical:
                        logger.warning(
                            f" Capsule {capsule['capsule_id']} has {len(critical)} critical contradictions"
                        )
                        for c in critical:
                            logger.warning(f"   [!] {c.category}: {c.description}")
                    elif warnings:
                        logger.info(
                            f" Capsule self-inspection: {len(warnings)} warnings, {len(infos)} info"
                        )
                else:
                    capsule["payload"]["self_inspection"] = {
                    ...
                    logger.debug(
                        f" Capsule {capsule['capsule_id']} passed self-inspection"
                    )

            except Exception as e:
                logger.debug(f"Contradiction detection skipped: {e}")

        # ADD PROOF LEVEL MARKERS to reasoning steps
        # This makes explicit what is verified vs. model-generated
        if _PROVENANCE_AVAILABLE and ProofLevel:
            try:
                for step in capsule["payload"].get("reasoning_steps", []):
                    # User messages are TOOL_VERIFIED (we captured them)
                    # Assistant messages are MODEL_GENERATED
                    if step.get("role") == "user":
                        step["proof_level"] = ProofLevel.TOOL_VERIFIED.value
                    else:
                        step["proof_level"] = ProofLevel.MODEL_GENERATED.value

                    # Confidence scores are HEURISTIC, not calibrated
                    if "confidence" in step:
                        step["confidence_proof"] = ProofLevel.HEURISTIC.value
