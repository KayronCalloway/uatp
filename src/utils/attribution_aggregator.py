...
def aggregate_attributions(
    ...
    for step_attr in step_attributions:
        ...
        for contrib in contributors:
            ...
            if aggregation_method == "weighted_sum":
                ...
                if timestamp:
                    try:
                        contrib_time = datetime.fromisoformat(
                            timestamp.replace("Z", "+00:00")
                        )
                        age_hours = (
                            datetime.now(timezone.utc) - contrib_time
                        ).total_seconds() / 3600
    ...
    if excess_weight > 0:
        uncapped_total = sum(
            w
            for aid, w in capped_weights.items()
    ...
    normalized_weights = {
        agent_id: weight / total_weight for agent_id, weight in capped_weights.items()
    }
