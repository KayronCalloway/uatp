...
class AgentExecutionService:
    ...
    async def complete_agent_session(
        ...
        try:
            ...
            if agent_session.started_at:
                duration_ms = int(
                    (
                        agent_session.completed_at - agent_session.started_at
                    ).total_seconds()
                    * 1000
                )
    ...
    async def get_session_stats(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        # Count by status
        status_query = select(
            AgentSessionModel.status, func.count(AgentSessionModel.id)
        ...
        # Count by agent type
        type_query = select(
            AgentSessionModel.agent_type, func.count(AgentSessionModel.id)
        ).group_by(AgentSessionModel.agent_type)
        type_result = await session.execute(type_query)
        by_agent_type = {row[0]: row[1] for row in type_result.fetchall()}

        # Total
        total_result = await session.execute(select(func.count(AgentSessionModel.id)))
    ...
    async def list_tool_calls(
        ...
        query = select(ToolCallModel).where(ToolCallModel.session_id == session_id)
    ...
    async def get_session_statistics(
        ...
        action_query = (
            ...
            .where(ActionTraceModel.session_id == session_id)
