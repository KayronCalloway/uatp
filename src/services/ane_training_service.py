...
class ANETrainingService:
    ...
    async def register_hardware_profile(
        ...
        try:
            ...
            logger.info(
                f"Registered hardware profile {profile_id} for {chip_identifier}"
            )
    ...
    async def get_profile_stats(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        # Count by chip
        chip_query = select(
            HardwareProfileModel.chip_identifier, func.count(HardwareProfileModel.id)
        ...
        class_query = select(
            HardwareProfileModel.device_class, func.count(HardwareProfileModel.id)
    ...
    async def get_session_stats(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        # Count by status
        status_query = select(
            ANETrainingSessionModel.status, func.count(ANETrainingSessionModel.id)
        ...
        return {
            ...
            "avg_ane_utilization": float(avg_row[1])
    ...
    async def get_session_statistics(
        ...
        return {
            ...
            "total_execution_time_us": int(total_row[1])
            if total_row and total_row[1]
            else 0,
    ...
    async def record_kernel_execution(
        ...
        try:
            ...
            ane_session.completed_steps = max(
                ane_session.completed_steps, step_index + 1
            )
    ...
    async def batch_record_kernel_executions(
        ...
        try:
            ...
            return {
                "success": False,
                "error": "Failed to batch record kernel executions",
            }
    ...
    async def list_kernel_executions(
        ...
        query = (
            query.order_by(
                KernelExecutionModel.step_index, KernelExecutionModel.dispatch_index
            )
            .offset(offset)
            .limit(limit)
        )
