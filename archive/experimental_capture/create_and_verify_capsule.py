...
async def create_and_verify():
    ...
    try:
        ...
        if all_present:
            ...
            print(
                f"  - Platform: {payload_data.get('metadata', {}).get('platform', 'N/A')}"
            )
            print(
                f"  - Envelope Version: {payload_data.get('metadata', {}).get('envelope_version', 'N/A')}"
            )
            print(
                f"  - Created By: {payload_data.get('metadata', {}).get('created_by', 'N/A')}"
            )
            print(
                f"  - Contributors: {len(payload_data.get('attribution', {}).get('contributors', []))}"
            )
            print(
                f"  - Generation: {payload_data.get('lineage', {}).get('generation', 'N/A')}"
            )
            print(
                f"  - Derivation Method: {payload_data.get('lineage', {}).get('derivation_method', 'N/A')}"
            )
            print()
        else:
