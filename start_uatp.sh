...
if pgrep -f "streamlit.*visualizer/app.py" > /dev/null; then
    echo "[OK] Visualizer already running at http://localhost:8501"
else
    echo " Starting Visualizer..."
    streamlit run visualizer/app.py --server.headless true --server.port 8501 > /dev/null 2>&1 &
    echo "[OK] Visualizer started at http://localhost:8501"
fi
...
echo "   [WARN]  API Server: Dependency issues - run 'pip install --upgrade redis' to fix"
echo ""
