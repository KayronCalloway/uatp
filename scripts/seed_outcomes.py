...
def extract_signals(payload: dict) -> dict:
    ...
    # Error indicators
    error_words = ["error", "failed", "exception", "broken", "wrong", "issue", "bug", "crash"]
    signals["has_error_mentions"] = any(w in text_content for w in error_words)
    
    # Success indicators
    success_words = ["success", "works", "fixed", "resolved", "complete", "implemented", "done"]
...
def infer_outcome(signals: dict) -> tuple:
    ...
    if conf >= 0.7 and signals["has_success_mentions"] and not signals["has_error_mentions"]:
        return ("success", 0.8 + random.uniform(-0.1, 0.15), "High confidence with success indicators")
    ...
    if conf >= 0.6 and signals["has_final_answer"] and signals["step_count"] >= 3:
        return ("success", 0.7 + random.uniform(-0.1, 0.1), "Complete reasoning with answer")
    
    # Mixed signals = partial
    if signals["has_error_mentions"] and signals["has_success_mentions"]:
        return ("partial", 0.65 + random.uniform(-0.05, 0.05), "Mixed success/error signals")
    
    # Default: use confidence as guide
    if conf >= 0.65:
        return ("success", conf * 0.9 + random.uniform(-0.05, 0.05), "Confidence-based inference")
    elif conf >= 0.45:
...
def seed_outcomes(db_path: str, limit: int = 50):
    ...
    for row in rows:
        ...
        """, (
            ...
            f"Auto-seeded: {notes}",
            json.dumps({"inference_confidence": round(conf, 3), "method": "heuristic_seeding"}),
            row["capsule_id"]
        ))
        outcomes[status] += 1
