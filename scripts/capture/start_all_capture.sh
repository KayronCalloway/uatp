...
if curl -s http://localhost:9090/health > /dev/null 2>&1; then
    ...
    echo "[ERROR] Backend not running - start with: ./start_backend_with_keys.sh"
    exit 1
fi
...
for platform, description in platforms:
    ...
    if response.ok:
        print(f'[OK] {platform}: service initialized')
    else:
        print(f'[ERROR] {platform}: initialization failed')
"
...
echo "[OK] Confidence ratings & reasoning traces"
echo "[OK] Economic attribution & value tracking"
echo "[OK] Trust metrics & compliance monitoring"
echo ""
