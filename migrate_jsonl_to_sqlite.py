...
This script migrates existing JSONL capsules to the SQLite database
...
async def perform_migration(jsonl_file: str):
    """Perform the actual migration."""

    print(f"🚀 Starting SQLite migration from {jsonl_file}")
...
async def verify_migration(jsonl_file: str):
    ...
    try:
        ...
        print(f"   Storage backend: {db_stats['storage_backend']}")
        print(f"   Platforms: {dict(db_stats.get('platforms', {}))}")
...
async def main():
    ...
    print("🚀 UATP Capsule Engine - JSONL to SQLite Migration")
    print("=" * 60)
    ...
    try:
        ...
        print(
            f"\n❓ Ready to migrate {analysis_stats['valid_capsules']} capsules to SQLite database?"
        )
        ...
        print(f"   Capsules are now stored in SQLite database")
        ...
        print(f"\n📝 Next Steps:")
        print(f"   1. Update real-time generator to use SQLite storage")
        print(f"   2. Test live capsule creation with database backend")
        print(f"   3. Verify capsule visualization works with database")
