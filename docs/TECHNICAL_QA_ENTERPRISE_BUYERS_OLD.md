...
1. [Deployment, Reliability & Ops](#1-deployment-reliability--ops)
2. [Security & Compliance](#2-security--compliance)
3. [Capsule Integrity & Crypto Details](#3-capsule-integrity--crypto-details)
4. [Integration Surface](#4-integration-surface)
5. [Enforcement & Governance](#5-enforcement--governance)
6. [Data Handling Nuances](#6-data-handling-nuances)
7. [Product Experience](#7-product-experience)
8. [Commercials & Risk](#8-commercials--risk)

---

## 1. Deployment, Reliability & Ops

### Q: What are your SLO/SLA targets (availability %, P95/P99 latency) and what are the credits if they're missed?
...
### Q: Throughput limits per deployment? Any rate-limiting on the proxy/SDK?
...
### Q: Disaster recovery: RPO/RTO, backup cadence, restore drills, region failover?
...
### Q: Canary/rollback steps for proxy/sidecar updates? Do you support blue/green?
...
### Q: Multi-env support (dev/stage/prod) and isolation guarantees?
...
### Q: Do you sign a BAA (HIPAA) and provide a DPA/SCCs (GDPR) out of the box?
...
### Q: Key management: Customer-managed keys (AWS KMS/GCP KMS/Azure Key Vault), key rotation, HSM support?
...
### Q: Supply-chain security: signed releases, SBOMs, reproducible builds, third-party dep audits?
...
### Q: Pen test cadence (quarterly/annual?), bug bounty, secure SDLC evidence?
...
### Q: Access control: SSO/SAML/OIDC, RBAC/ABAC, SCIM provisioning, break-glass procedures and audit of admin actions?
...
### Q: Algorithm agility for PQC (e.g., Dilithium parameter sets, migration plan if NIST guidance changes)?
...
### Q: How do you handle clock skew when chaining across distributed services?
...
### Q: Partial failure semantics (e.g., request succeeds but capsule write fails) and idempotency?
...
### Q: Can you produce batch attestations (Merkle roots) and anchor them to an external timestamping service?
...
### Q: Salted hashing / collision resistance for "content fingerprints"; guidance to avoid sensitive-data reconstruction from metadata?
...
### Q: Full matrix of supported providers & features: OpenAI/Anthropic/Vertex/Azure, streaming, tool/function-calling, images, RAG, batch APIs?
...
### Q: Non-Python stacks before JS GA: gRPC/REST shim examples, Terraform/Helm modules, sidecar for serverless?
...
### Q: Observability hooks: Prometheus metrics, OpenTelemetry traces, Splunk/Datadog dashboards "ready to import"?
...
### Q: API/versioning policy and deprecation timelines?
...
### Q: False-positive/false-negative tracking for circuit breakers; do you expose metrics and a review queue?
...
### Q: Override workflow: who can override, MFA, justification capture, and reporting—can we make override SLOs and alerts?
...
### Q: Policy-as-code: language, testing framework, CI integration, staging "shadow" mode before enforce?
...
### Q6.1: What exactly is in the capsule metadata schema, and can it change backward-incompatibly?
...
### Q6.2: Can we enforce data residency at a finer grain (per-workflow or per-tenant)?
...
### Q6.3: How do you handle GDPR data subject requests (Article 15 for access, Article 17 for deletion)?
...
### Q6.4: What about retention policies and automated deletion?
...
### Q7.1: Is there a dashboard for searching chains, verifying signatures, and generating reports?
...
### Q7.2: Can we set up alerting on policy violations, refusals, or tamper detection?
...
### Q7.3: Are there pre-built template packs (HIPAA, GDPR, PCI, EU AI Act)?
...
### Q7.4: Is there onboarding documentation, runbooks, and training available?
...
### Q8.1: What protections exist against price increases or unexpected charges?
...
90-Day Pilot:
  ...
  - Included: Unlimited policy deployments, audit reports, support
  - Overage: If you exceed 100K interactions, we discuss before charging
...
```
Annual Subscription:
  Base Price: $36,000/year (300K interactions/month)

  Price Protection:
    - Renewal increase cap: 10% annually (max)
    - Interaction overages: $0.10 per interaction over plan
    - Overage cap: 25% of base (auto-upgrade to next tier after)

  Example:
    Year 1: $36K base + $5K overages = $41K total
    Year 2 renewal: $36K → max $39.6K base (10% cap)

  Contractual: Price caps written into MSA (Master Service Agreement)
```

**3. Volume Discounts (Locked In):**
```
Volume Tiers:
  - 0-300K interactions/month: $36K/year ($0.10/interaction)
  - 300K-1M interactions/month: $60K/year ($0.05/interaction)
  - 1M-5M interactions/month: $180K/year ($0.03/interaction)
  - 5M+ interactions/month: Custom pricing (negotiated, locked for 3 years)

  Tier Lock: Once you reach a tier, you stay there even if usage drops
```
...
Extra Costs (Transparent):
  ...
  - Custom policy development: $5K-$15K per policy (one-time)
...
Termination Terms:
  ...
  - Annual contract: 90-day notice, no penalty (after month 6)
  - Multi-year: Annual breakpoints, no penalty (after year 1)

  Data Export:
    - 60 days to export all capsules (included)
    - Export format: JSON, CSV, or direct DB dump
    - Verification scripts included
```

**Honest Assessment:**
> Pricing is straightforward for pilot, but enterprise pricing gets complex if you have multi-region, multi-tenant, or custom SLA requirements. We quote those separately. Standard pricing is transparent and contractually protected.

---

### Q8.2: Can we speak to a reference customer before committing?
...
```
Available Now:
  - Live demo of the system working end-to-end
  - Run demo_audit_artifacts.py to see actual output
  - Review generated audit reports (sample data)
  - Examine cryptographic verification in action
  - Test the system in your environment during pilot
```

**2. Code Review:**
```
Available for Technical Due Diligence:
  - Full codebase review (363 production modules)
  - Architecture deep dive with engineering team
...
During Your Pilot:
  ...
  - 90 days to validate everything works
  - No payment until you see it working
  - If it doesn't work, you part ways with data export
```
...
If It Works For You:
  ...
  - Options: Anonymous case study, public case study, reference calls
  - Incentive: 10% discount on year 1 if you participate

Reference Program Benefits (For First Customers):
  - Priority roadmap input
  - Early access to new features
  - Co-marketing opportunities (if desired)
  - "First customer" positioning advantage
```

**Honest Assessment:**
> We have zero customers. If you need 3 production references with multi-year track records, we can't provide that. But if you want first-mover advantage and are willing to validate a new technology during a 90-day pilot, that's the opportunity. You'd be taking the risk of being first, with the upside of shaping the product and getting favorable terms.

---

### Q8.3: What are the proof milestones in the pilot to validate the system works?
...
Milestone 2: Policies Detecting Issues (Not Blocking Yet)
  ...
  Proof:
    - Dashboard shows: "234 violations detected, 0 blocked (shadow mode)"
    - Zero production impact (latency graphs)
```
...
Milestone 3: Policies Enforcing in Staging/Canary
  ...
  Proof:
    - Demonstrate live: "This request should be blocked... [blocked]"
    - Show capsule: Refusal reason captured cryptographically
```
...
Milestone 4: Audit Artifacts Generated for Regulator
  ...
  Deliverable:
    ...
    - Insurance actuarial data (policy enforcement metrics)

  Proof:
    - You present this report to your legal/compliance team
    - They confirm: "This is what we'd give to a regulator"
```
...
Milestone 5: Full Production Deployment Plan
  ...
  Proof:
    - You decide: "We're comfortable going to 100% enforcement"
```
...
Pilot Success Outcomes:
  1. Convert to annual subscription ($36K/year)
  2. Expand to more workflows
  3. Get first audit report in regulator's hands
  4. Become reference customer (if willing)

Pilot Failure Outcomes:
  1. We work with you to fix gaps (free extension)
  2. If unfixable, we part ways (no hard feelings)
  3. You keep all capsules and audit artifacts
```

**Honest Assessment:**
> The pilot is structured to derisk your decision. By week 6, you'll know if this works. By week 12, you'll have concrete proof. If we can't deliver milestones, we extend the pilot (no extra cost) until we do.

---

### Q8.4: If we decide UATP isn't right for us, what's the offboarding plan?
...
```
Notice Period:
  - Pilot: 30 days' notice
  - Annual: 90 days' notice
...
Data Export Options:
  ...
  Option A: Full Database Dump
    ...
    - Includes: All signatures, policies, audit logs
    - Format: SQL dump file (gzipped)
    - Size: Depends on usage (typically 10-500 GB)

  Option B: JSON Export
    - One JSON file per capsule chain
    - Organized by workflow and date
    - Includes: Verification scripts

  Option C: CSV Export
    - Flattened data for import to other systems
    - Includes: Capsule metadata, policy results, timestamps

  Option D: API Access
    - Keep API access for 60 days post-termination
...
Independent Verification:
  - We provide standalone verification tool
  - Verifies capsule signatures WITHOUT UATP backend
...
```
Data Deletion (Your Choice):

  Option 1: Delete Immediately
    - We delete all your data from UATP servers
    - Secure overwrite (DoD standard)
    - Deletion certificate provided

  Option 2: Archive for 1 Year
    - We archive capsules (read-only)
    - $500/month archival fee
    - Gives you time to migrate slowly

  Option 3: Transfer to Your Infrastructure
    - We help you deploy UATP on your servers (self-hosted)
    - One-time migration fee: $10K
    - You take over operation and maintenance
```
...
Offboarding Guarantees (In MSA):
  - No data hostage: You get full export, period.
  - No format lock-in: JSON/CSV/SQL exports (standard formats)
  - No ransom fees: Export is included in base price
  - No forced retention: We delete when you ask (with proof)
  - No post-termination charges: Except archival (optional)
```

**Self-Hosted Option (Keep Using UATP Without Us):**
```
UATP Open Core Model:
  - Core engine: Open source (GitHub)
  - You can self-host after termination
  - We provide migration assistance (paid service)
  - Lose: Managed service, support, updates
  - Keep: All functionality, all data, full control
```

**Why Offboarding Might Happen:**
```
Anticipated Reasons (No Customer Data Yet):
  1. "Too early for us" → Market timing not right
  2. "Integration too complex" → Implementation challenges
  3. "Cost not justified yet" → ROI timing mismatch
  4. "Regulatory requirements changed" → External factors
  5. "Built in-house alternative" → Make vs. buy decision

Note: We have no actual offboarding experience yet since we have no customers.
These are anticipated scenarios based on enterprise software patterns.
```

**Honest Assessment:**
> We want satisfied customers, not hostages. If UATP doesn't work for you, we'll help you leave cleanly. No data lock-in, no vendor hostage tactics. Our bet is that if we're good, you'll stay or come back.
...
1. **Deployment, Reliability & Ops** - SLOs, throughput, DR, multi-env
2. **Security & Compliance** - BAAs, key management, pen testing, access control
3. **Capsule Integrity & Crypto** - PQC, clock skew, Merkle trees, salted hashing
4. **Integration Surface** - Provider support, observability, API versioning
5. **Enforcement & Governance** - False positives, overrides, policy-as-code
6. **Data Handling Nuances** - Schema versioning, GDPR requests, retention, residency
7. **Product Experience** - Dashboard, alerting, templates, training
...
- Reference during technical deep dives
- Use as FAQ for pilot customers
- Update based on real customer questions

**Document Location:**
`/Users/kay/uatp-capsule-engine/docs/TECHNICAL_QA_ENTERPRISE_BUYERS.md`

---

**Document Complete.** All 8 sections finished.