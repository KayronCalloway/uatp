...
**Purpose:** Detect implicit user feedback from conversation patterns

**Location:** `src/live_capture/signal_detector.py`

**Signal Types Detected:**
| Signal | Detection Method | Meaning |
|--------|-----------------|---------|
| `correction` | "no", "that's wrong", "actually" | Response was incorrect |
| `acceptance` | "thanks", "perfect", "that works" | Response was helpful |
| `refinement` | "can you also", "what about" | Response good, user wants more |
| `requery` | Similar question repeated (similarity > 0.5) | Response didn't help |
| `abandonment` | "never mind", "forget it" | User gave up |
| `code_execution` | "I ran", "output is" | User trusted and ran code |

**Key Classes:**
```python
from src.live_capture.signal_detector import SignalDetector, Signal, SessionSignals

detector = SignalDetector()
signal = detector.detect_signal("Thanks, that works!", previous_msgs=[], role="user")
# Signal(signal_type="acceptance", confidence=0.8, sentiment_delta=0.6, ...)
```

**Session Aggregation:**
Signals are aggregated into `session_metadata.feedback_signals`:
```json
{
  "feedback_signals": {
    "correction_count": 1,
    ...
    "acceptance_rate": 0.5,
    "net_sentiment": 0.45,
    "inferred_reward": 0.83
  }
}
```

**Frontend Display:**
- `FeedbackSignalsCard` component shows quality score and signal breakdown
- Signal type badges appear on reasoning steps in capsule detail view
